...
/*
 +-------------------------------------------------------------------------+
 | User Interface for smime_verify plugin                                    |
 |                                                                         |
...
class enigma_ui
{
    ...
    {
        $this->enigma->include_script('smime_verify.js');

        // Enigma actions
        if ($this->rc->action == 'plugin.smime_verify') {
        ...
        // Preferences UI
            ...
            if ($section == 'smime_verify_certs') {
                $this->rc->output->add_handlers(array(
                    'keyslist' => array($this, 'template_certs_list'),
                    'keyframe' => array($this, 'template_cert_frame'),
                    'countdisplay' => array($this, 'template_certs_rowcount'),
                    'searchform' => array($this->rc->output, 'search_form'),
                ));
                $this->rc->output->set_pagetitle($this->enigma->gettext('smime_verify_certs'));
                $this->rc->output->send('smime_verify.certs'); 
            }
            else {
                $this->rc->output->add_handlers(array(
                    'keyslist' => array($this, 'template_keys_list'),
                    'keyframe' => array($this, 'template_key_frame'),
                    'countdisplay' => array($this, 'template_keys_rowcount'),
                    'searchform' => array($this->rc->output, 'search_form'),
                ));
   ...
   /**
    ...
    function add_css()
    {
        ...
        if (!file_exists($this->home . "/skins/$skin/smime_verify.css"))
            $skin = 'default';

        $this->enigma->include_stylesheet("skins/$skin/smime_verify.css");
        $this->css_added = true;
    }
     ...
     */
    function template_key_frame($attrib)
    {
        if (!$attrib['id']) {
     ...
     */
    function template_keys_list($attrib)
    {
...
//        $a_show_cols = array('name');
     ...
     * @return string HTML output
     */
    function template_keys_rowcount($attrib)
    {
        if (!$attrib['id'])
     ...
     */
    function template_key_name($attrib)
    {
     ...
     */
    function template_key_data($attrib)
    {
        $out = '';
     ...
     */
    function template_key_import_form($attrib)
    {
